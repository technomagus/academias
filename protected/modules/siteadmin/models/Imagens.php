...
class Imagens extends CActiveRecord
{
	...
	public function adminAlbum($id, $tipo)
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.
		...
		$criteria->compare('im_titulo',$this->im_titulo,true);
		$criteria->compare('im_descricao',$this->im_descricao,true);
                $criteria->condition = 'im_id_estran=' .$id. ' AND im_tipo="'.$tipo.'"';

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
        public function bSalvaImagem($img)
        {
            Yii::import('application.extensions.image.Image');
            $image = new Image($img->tempName);
            $image->resize('480', '640', IMAGE::AUTO);
            $image->save($img->tempName . '.'.$img->extensionName);
            $ctudo = file_get_contents($img->tempName . '.'.$img->extensionName);
            $imagem = 'data:'.$image->mime.';base64,'.base64_encode($ctudo);
            unlink($img->tempName .'.'.$img->extensionName);
            
            $this->im_imagem = $imagem;
        }
        ...
        public function nGetIdEstran()
        {
            return $this->im_id_estran;
        }
}